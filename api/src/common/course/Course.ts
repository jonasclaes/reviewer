...
export class Course implements ICourse {
    ...
    public static async findAll(limit: number, offset: number): Promise<Course[]> {
        return new Promise((resolve, reject) => {
            database.getConnection().query("SELECT * FROM `courses` LIMIT ? OFFSET ?", [limit, offset], (err, results: ICourse[], fields) => {
                if (err) {
                    reject(err);
                } else {
